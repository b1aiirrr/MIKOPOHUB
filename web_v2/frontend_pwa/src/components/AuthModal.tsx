...
import { LogIn, UserPlus, Shield, User, X, Check, Lock, Smartphone, CreditCard, Mail } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = isLogin
        ? { username, password }
        : { username, email, password, full_name: fullName, phone, national_id: nationalId };
  ...
  const handleGoogleLogin = async () => {
    ...
    setError(null);
    try {
      const googleEmail = prompt('Enter your Gmail address:', 'borrower@gmail.com');
      if (!googleEmail) {
        setLoading(false);
        return;
      }
      ...
      const res = await fetch('http://localhost:8000/api/auth/google-login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: googleEmail, full_name: googleName }),
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="relative w-full max-w-md bg-[#161922] border border-[#2a2f3d] rounded-2xl shadow-2xl overflow-hidden p-6 space-y-5">
        ...
        <button
          ...
          className="w-full bg-[#0b0d12] hover:bg-slate-800 text-slate-200 border border-slate-700 py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-2.5 transition-all shadow"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div>
            <label className="text-xs font-semibold text-slate-300 block mb-1">
              {isLogin ? 'Username or Gmail Address' : 'Username'}
            </label>
            <input
              type="text"
              required
              ...
              placeholder={isLogin ? "admin or user@gmail.com" : "e.g. john_doe"}
